...
<template>
    <div class="min-h-screen flex bg-gray-50">
        <!-- Sidebar -->
        <aside class="sticky top-0 h-screen shrink-0 border-r bg-white/80 backdrop-blur px-2 py-4 transition-all"
            :class="[asideWidth]">
            <!-- 内部は縦並び・全高で配置制御 -->
            <div class="flex h-full flex-col">
                ...
                <nav class="flex flex-col gap-2">
                    <NuxtLink v-for="item in nav" :key="item.to" :to="item.to"
                        class="group rounded-xl px-2 py-2 text-sm transition flex items-center gap-3" :class="isActive(item.to)
                            ? 'bg-black text-white'
                            : 'text-gray-700 hover:bg-gray-100'" :title="itemTitle(item.label)">
                        <Icon :name="item.icon" class="text-lg shrink-0" />
                        <span v-if="!collapsed" class="truncate">{{ item.label }}</span>
                    </NuxtLink>
                </nav>
                <div class="mt-auto border-t pt-3 px-2">
                    <button class="group w-full rounded-lg px-2 py-2 hover:bg-gray-100 text-gray-700
           flex items-center gap-2 transition focus-visible:outline-none
           focus-visible:ring-2 focus-visible:ring-black/30" :class="collapsed ? 'justify-center' : 'justify-start'"
                        :aria-label="collapsed ? 'サイドバーを展開' : 'サイドバーを折りたたむ'"
                        :title="collapsed ? 'サイドバーを展開' : 'サイドバーを折りたたむ'" @click="collapsed = !collapsed">
                        <!-- アイコンを MDI の円形 chevron に。ホバーで少し拡大 -->
                        <Icon :name="collapsed ? 'mdi:chevron-right-circle' : 'mdi:chevron-left-circle'"
